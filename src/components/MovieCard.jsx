...
import { posterUrl, formatYear, formatRating, truncate } from '../utils/helpers'
      ...
      <div className="bg-cinema-800 rounded-xl overflow-hidden border border-white/5">
        ...
        <div className="relative aspect-[2/3] overflow-hidden bg-cinema-700">
          {!imgError && poster ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-cinema-700">
              ...
              <span className="text-white/20 text-xs font-mono text-center px-4">{movie.title}</span>
            </div>
          )}

          {/* Rating badge */}
          {movie.vote_average > 0 && (
            <div className={`absolute top-2 right-2 flex items-center gap-1 bg-cinema-950/80 backdrop-blur-sm rounded-md px-2 py-1 ${ratingColor}`}>
              <svg className="w-3 h-3 fill-current" viewBox="0 0 24 24">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
              </svg>
              <span className="text-xs font-mono font-medium">{formatRating(movie.vote_average)}</span>
            </div>
          )}

          {/* Year tag */}
          <div className="absolute bottom-2 left-2 bg-cinema-950/80 backdrop-blur-sm rounded px-2 py-0.5">
            <span className="text-white/60 text-xs font-mono">{formatYear(movie.release_date)}</span>
          </div>
        ...
        <div className="p-3">
          ...
          <div className="flex items-center justify-between mt-1">
            <span className="text-white/30 text-xs font-mono">{formatYear(movie.release_date)}</span>
            {movie.vote_count > 0 && (
              <span className="text-white/20 text-xs font-mono">{movie.vote_count.toLocaleString()} votes</span>
            )}
          </div>
        </div>
      </div>
